...
# project imports
from ..raspi_python import wake_word
from ..raspi_python import speech_to_text
from ..raspi_python import conversation_gemini
from ..raspi_python import text_to_speech
...
def start_voice_assistant(app): #Gemini Voice Assistant & Set Input textbox with Gemini output
    wake_word_text = "hey weezy"  # Wake word
    ...
    while not stop_event.is_set():
        ...
        if bool_wake:
            # Speech-to-text  & Send input to Gemini Using Internet Connection
            command = speech_to_text.recognize_speech(mic_index)
            output = conversation_gemini.start_prompt(command)
            # Update text box with the response from AI voice assistant
            app.input_text.set(output)
            # gemini output to text-to-speech
...
def main():
    ...
    voice_assistant_thread = threading.Thread(target=start_voice_assistant, args=(app,))
